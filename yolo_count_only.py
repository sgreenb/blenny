...
def bootstrap():
    # find_spec avoids importing the package just to test availability (a bare
    # import here would be flagged F401 "unused import" by linters).
...
bootstrap()
# Import must stay below bootstrap() so the auto-install can run first when
# ultralytics is missing (hence the noqa: E402).
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Professional Colony Counter CLI", prefix_chars="-"
    )
    ...
    parser.add_argument(
        "-model",
        default="models/colony_model.pt",
        help="Path to model file (default: models/colony_model.pt)",
    )
