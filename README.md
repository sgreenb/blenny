...
blenny run pipeline_yolo_facile.yaml --input plate.jpg --output results/
```
...
1. Use `pipeline_yolo_facile_grid.yaml`.
2. Run via CLI:
   ```bash
   blenny run pipeline_yolo_facile_grid.yaml -i scan.jpg -o out/ -v detect_multi_plate.grid=[2,3]
   ```
...
pipe = Pipeline.from_yaml("pipeline_yolo_facile.yaml")
