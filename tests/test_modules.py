...
def test_circularity_filter_drops_elongated_shapes() -> None:
    ...
    out = ThresholdSegmenter(roi_mask_key=None, split_touching=False, min_circularity=0.7).run(data)
    labels = out.masks["objects"]
...
def test_solidity_filter_drops_irregular_shapes() -> None:
    ...
    data = ImageData(source="x", image=img)
    out = ThresholdSegmenter(roi_mask_key=None, split_touching=False, min_circularity=0.0, min_solidity=0.9).run(data)
