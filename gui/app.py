...
with st.sidebar:
    ...
    output_folder_input = c_o1.text_input(
        ...
        help="Leave empty to run analysis — the folder is only required when saving/exporting results.",
    )
    ...
    if input_paths:
        ...
        with col2:
            ...
            if st.session_state.get("all_results"):
                ...
                if c_nav1.button("←", disabled=idx == 0, width="stretch"):
                    st.session_state["current_view_idx"] -= 1
                    st.rerun()
                st.session_state["current_view_idx"] = stems.index(
                    c_nav2.selectbox("Plate", stems, index=idx, label_visibility="collapsed")
                )
                if c_nav3.button("→", disabled=idx == len(stems) - 1, width="stretch"):
                    st.session_state["current_view_idx"] += 1
                    st.rerun()
                ...
                if output_folder_input.strip():
                    ...
                    save_dir = None
                if st.button(
                    "Save All Results to Output Location", type="primary", width="stretch"
                ):
                    if save_dir is None:
                    ...
                    for d, p in st.session_state.get("batch_runs", []):
                        ...
                        for step in p.steps:
                            if hasattr(step, "export"):
                                step.export(d)
                        if old_out:
                            d.metadata["output_dir"] = old_out
                    generate_batch_summary([d for d, _ in st.session_state["batch_runs"]], save_dir)
                    generate_batch_colonies(
                        [d for d, _ in st.session_state["batch_runs"]], save_dir
                    )
