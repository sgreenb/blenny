...
def batch_summary_text(batch_data):
    ...
    nothing to summarize, mirroring generate_batch_summary's early returns)."""
    import csv
    import io
...
def batch_colonies_text(batch_data):
    """Return the batch_colonies.csv content as a string."""
    if not batch_data:
        return None
    all_measurements = []
    for data in batch_data:
...
def export_results_dialog(
    ...
    csv_text: str,
    log_text: str,
    annot_bytes: bytes | None,
    batch_csv_text: str | None,
    batch_summary_text: str | None,
) -> None:
    """Modal dialog letting the user pick which files to export, rename them,
    then download them individually or write them to an output folder. Only
    files actually generated by this analysis run are listed (batch files are
    offered only when multiple input images were processed). Widget keys are
    reset by the caller before opening, so the dialog always starts from the
    default names/selection."""
    st.caption(f"Current plate: **{stem}**")
    files = [
        ("annot", "Annotated image (PNG)", f"{stem}_annotated.png", annot_bytes, "image/png"),
        ("csv", "Colonies CSV", f"{stem}_colonies.csv", csv_text, "text/csv"),
        ("log", "Processing log (TXT)", f"{stem}_log.txt", log_text, "text/plain"),
    ...
    selected = []  # (filename, payload, mime, is_batch)
    for kind, label, default_name, content, mime in files:
        if content is None:
            # Not generated in this run (e.g. batch files for a single image,
            # or an annotated image that could not be rendered) — don't offer it.
            continue
        c1, c2, c3 = st.columns([0.5, 2.1, 0.8], vertical_alignment="center")
        include = c1.checkbox(label, value=True, key=f"export_include_{kind}")
        name = c2.text_input(
            "File name",
            value=default_name,
            key=f"export_name_{kind}",
            label_visibility="collapsed",
        )
        c3.download_button(
            "Download",
        ...
        if include:
            selected.append(
                (name or default_name, _payload(content), mime, kind.startswith("batch_"))
            )
    ...
    if st.button("Save to Output Folder", type="primary", width="stretch"):
        ...
        if not out:
            ...
            for fname, payload, _mime, is_batch in selected:
                dest_dir = save_dir if (is_batch or not use_subfolders) else save_dir / stem
                ...
                try:
                    ...
                    with st.status("Analyzing...", expanded=True) as status:
                        ...
                        generate_batch_summary(
                            [d for d, _ in st.session_state["batch_runs"]], output_dir
                        )
                    ...
                    if st.button("Export Results", type="primary", width="stretch"):
                        ...
                        for _k in list(st.session_state):
                            if _k.startswith("export_"):
                                del st.session_state[_k]
                        # Batch files exist only when more than one input image
                        # was processed (matching the CLI's default).
                        batch_data = [d for d, _ in st.session_state.get("batch_runs", [])]
                        has_batch = len(batch_data) > 1
                        export_results_dialog(
                            stem=stem,
                            csv_text=csv_exporter.generate_csv(data),
                            log_text=summarizer.generate_text(data),
                            annot_bytes=_ensure_annot_cache(),
