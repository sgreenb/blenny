...
def create_plate(draw, cx, cy, r, label, colony_range=(10, 35)):
    ...
    for i in range(num):
        ...
        # Edge colonies sit 2-8 px inside the agar edge; the rest spread across the plate.
        dist = r - random.uniform(2, 8) if i < 8 else np.sqrt(random.uniform(0, 1)) * (r - 12)
...
def generate_diverse_testset(out_dir="sandbox/diverse_tests"):
    ...
    for name, w, h, rows, cols, r, style in scenarios:
        ...
        cell_w, cell_h = (
            w // (cols if "corner" not in style else 4),
            h // (rows if "corner" not in style else 4),
        )
